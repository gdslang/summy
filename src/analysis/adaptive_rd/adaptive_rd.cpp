...
void adaptive_rd::add_constraint(size_t from, size_t to, const edge *e) {
  ...
  auto id_assigned = [&](int_t size, variable const *v) {
    ...
    transfer_f = [=]() {
      //    cout << "assignment handler for edge " << node_id << "->" << dest_node << ", input
      //    state: " << *state[node_id] << endl;
      auto acc = state[from];
  ...
  ev._([&](const stmt_edge *edge) {
    ...
    v._([&](store const *s) {
      transfer_f = [=]() { return shared_ptr<adaptive_rd_state>(state[from]->set_memory_rev(to)); };
    });
    stmt->accept(v);
  });
