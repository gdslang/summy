...
num_linear *converter::conv_linear(linear const *lin, int64_t scale) {
  ...
  lv._(
    [&](lin_scale const *l) { result = conv_linear(&l->get_operand(), scale * l->get_const()); });
  lv._([&](lin_var const *l) {
    num_linear *transLE_lin =
      transLE(shared_copy(&l->get_var().get_id()), l->get_var().get_offset(), size);
...
expr_cmp_result_t analysis::api::converter::conv_expr_cmp(gdsl::rreil::sexpr const *se) {
  ...
  sv._([&](sexpr_cmp const *x) { result = conv_expr_cmp(x); });
...
analysis::api::num_expr *converter::conv_expr(sexpr const *se) {
  ...
  sexpr_visitor sv;
  sv._([&](arbitrary const *x) { result = new num_expr_lin(new num_linear_vs(value_set::top)); });
  sv._([&](sexpr_cmp const *x) {
    expr_cmp_result_t r_x = conv_expr_cmp(x);
    result = r_x.primary->copy();
    r_x.free();
  });
  sv._([&](sexpr_lin const *x) { result = new num_expr_lin(conv_linear(&x->get_lin())); });
  se->accept(sv);
  return result;
}
...
analysis::api::num_expr *converter::conv_expr(gdsl::rreil::expr const *expr) {
  ...
  ev._([&](expr_binop const *e) {
    ...
    auto rfo = [&](auto o) { result = new num_expr_bin(nl_opnd1, o, nl_opnd2); };
    auto rfo_signed = [&](
      auto o) { result = new num_expr_bin(nl_opnd1, o, nl_opnd2, sign_interp_t(SIGNED, size)); };
    switch(e->get_op()) {
      case BIN_MUL: {
        rfo(MUL);
        break;
  ...
  ev._([&](expr_sexpr const *e) { result = conv_expr(&e->get_inner()); });
  expr->accept(ev);
  return result;
}
...
num_linear *analysis::api::converter::mul(int64_t scale, num_linear *a) {
  ...
  nv._([&](num_linear_term *v) {
    result =
      new num_linear_term(scale * v->get_scale(), v->get_var()->copy(), mul(scale, v->get_next()));
  });
...
num_linear *converter::add(num_linear *a, vs_shared_t vs) {
  ...
  nv._([&](num_linear_vs *v) { result = new num_linear_vs(*vs + v->get_value_set()); });
  a->accept(nv);
  return result;
}
...
num_linear *converter::add(num_linear *a, num_linear *b) {
  ...
  nv._([&](num_linear_vs *v) { result = add(b, v->get_value_set()); });
  a->accept(nv);
  return result;
}
