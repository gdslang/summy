...
shared_ptr<summary_memory_state> analysis::summary_dstack_stubs::allocator(size_t allocation_site, size_t size) {
  ...
  field f_out = rax_region.retrieve_field(malloc_summary->child_state, 0, 64, false, true).f.value();
  ...
  field sp_q = sp_region.retrieve_field(malloc_summary->child_state, 0, 64, false, true).f.value();
